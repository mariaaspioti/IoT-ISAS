...
route = np.array([ [ -6.52506527964661, 53.37615438242526], 
...
smart_lock_2_id = 'urn:ngsi-ld:Device:94'
serial_number_2 = '14990868245'
...
def scenario_beacon(mqtt_client, route, device_id, base_topic, stop_event):
  ...
  while not stop_event.is_set():   
    ...
    if (route[i][1] == 53.37632079206085 and route[i][0] == -6.524829189502535):
      with nfc_lock:  # Ensure only one thread can enter
        if not nfc_fab1_executed.is_set():  # Ensure it's only executed once
          ...
          nfc_fab1_executed.set()  # Mark as executed, so no other thread runs it
      time.sleep(13)

    # Second NFC trigger condition (Cleanroom 1)
    if (route[i][1] == 53.37698002384256 and route[i][0] == -6.526057906047545):
      with nfc_lock:  # Ensure only one thread can enter
        if not nfc_cleanroom1_executed.is_set():  # Ensure it's only executed once
          ...
          nfc_cleanroom1_executed.set()  # Mark as executed, so no other thread runs it
      time.sleep(5)
