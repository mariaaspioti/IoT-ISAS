...
SCALE = 25  # Pixels per simulation unit
...
def simulation_setup():
    ...
    beacon_positions = []
    spacing = 4
    grid_size = int(np.sqrt(num_beacons))  # Assuming a square grid for simplicity
    for i in range(grid_size):
        for j in range(grid_size):
            beacon_positions.append([i * spacing, j * spacing])
...
def run_simulation(trail_enabled=False):
    ...
    # Parameters for Bluetooth tracking
    bt_transmission_interval = 1000 / 2  # quadruple the time
    bt_estimation_interval = 1000 / 2  # quadruple the time
    bt_backend_update_interval = 5000 / 2  # quadruple the time

    # Parameters for GPS tracking
    gps_transmission_interval = 5000 / 2  # quadruple the time
    ...
    while running:
        ...
        # Determine intervals based on tracking method
        if tracking_method == "Bluetooth":
            transmission_interval = bt_transmission_interval
            estimation_interval = bt_estimation_interval
            backend_update_interval = bt_backend_update_interval
        else:
            transmission_interval = gps_transmission_interval
            estimation_interval = gps_estimation_interval
            backend_update_interval = gps_backend_update_interval
        ...
        # Calculate the inner simulation time, which is quadruple the real time
        inner_time = (current_time - start_time) * 2 // 1000
        ...
        screen.blit(inner_timer_text, (150, 10))
        screen.blit(tracking_method_text, (10, 40))
        screen.blit(error_text, (10, 70))
