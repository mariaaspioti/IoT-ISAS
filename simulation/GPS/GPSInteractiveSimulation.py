...
import numpy as np
import time
from BeaconReceiverClasses import Beacon, StoredData
...
def simulation_setup():
    ...
    return beacons, gps_receiver, storedData
...
def run_simulation(trail_enabled=False):
    ...
    pygame.display.set_caption("GPS Positioning Simulation")
    ...
    # Setup
    beacons, gps_receiver, storedData = simulation_setup()
    beacon_positions = StoredData.set_beacon_positions(storedData, beacons)

    # Parameters
    transmission_interval = 250
    estimation_interval = 250
    backend_update_interval = 1000
    ...
    true_position = np.array([gps_receiver.x, gps_receiver.y])
    ...
    while running:
        ...
        if current_time - last_estimate >= estimation_interval:
            ...
            accumulated_estimations.append(gps_reading)
